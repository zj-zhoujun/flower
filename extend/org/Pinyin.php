...
class Pinyin
{
     ...
     * @return string
     */
    public static function getInitial($string, $encoding = 'utf-8')
    {
        if($string != self::$string || $encoding != self::$encoding) {
            self::chineseToPinyin($string, $encoding);
        }
        $initial = self::$short_pinyin;

        return strtoupper(mb_substr($initial, 0, 1));
    }
     ...
     * @return array
     */
    public static function setArrToInitial($arr = Array())
    {
        $newArr = array();
        foreach($arr as  $v) {
            $init = self::getInitial($v);
            $newArr[$init][] = $v;
        }
        ksort($newArr);
        return $newArr;
